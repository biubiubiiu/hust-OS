...
void getTime(char *buffer) {
    ...
    time(&t);
    lt = localtime(&t);
    sprintf(buffer, "%s%d/%s%d%s/%d %d:%s%d:%s%d\n",
            "最后一次打开和运行时间是: ", lt->tm_year + 1900,
            lt->tm_mon + 1 < 10 ? "0" : "", lt->tm_mon + 1,
            lt->tm_mday < 10 ? "0" : "", lt->tm_mday, lt->tm_hour,
            lt->tm_min < 10 ? "0" : "", lt->tm_min, lt->tm_sec < 10 ? "0" : "",
            lt->tm_sec);
}
...
int main() {
    ...
    while (fgets(cmdLine, MAX_LEN, stream) != NULL) {
        ...
        if (cmdLine[p] == '\n') cmdLine[p] = '\0';
        // 创建进程
        CreateProcess(NULL, cmdLine, NULL, NULL, TRUE, CREATE_NEW_CONSOLE, NULL,
                      NULL, &si, &pi);
    }
